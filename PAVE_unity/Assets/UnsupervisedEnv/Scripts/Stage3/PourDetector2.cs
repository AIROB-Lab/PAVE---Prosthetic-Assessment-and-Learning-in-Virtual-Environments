...
// based on https://www.youtube.com/watch?v=hyiyjUEReYg
{
    ...
    public static bool isPouring2 = false;
    private Stream2 currentStream = null;
    ...
    private void Update()
    {
        ...
        if(LevelManager.level == Levels.PourIngredients && LevelManager.placed)
        {
            ...
            if (isPouring2 != pourCheck)
            {
                isPouring2 = pourCheck;
                if (isPouring2)
                {
                    StartPour();
                }
    ...
    private Stream2 CreateStream()
    {
        GameObject streamObject = Instantiate(streamPrefab, origin.position, Quaternion.identity, transform);
        return streamObject.GetComponent<Stream2>();
    }
}